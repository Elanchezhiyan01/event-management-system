...
def update_event(event_id, data):
    ...
    # We only care about tracking venue, date, and time for emails.
...
def delete_event(event_id):
    """
    Deletes an event. 
    Delete Policy: Deletion behavior relies on the schema's 'ondelete="CASCADE"' rule 
    on the Registration model. This means deleting an event automatically drops registrations.
    However, if an integrity error or other exception occurs, it is rolled back and raised.
    """
    event = get_event_by_id(event_id)
    try:
        ...
        db.session.rollback()
        raise EventOperationError("An error occurred while deleting the event. It might have active registrations.") from e
